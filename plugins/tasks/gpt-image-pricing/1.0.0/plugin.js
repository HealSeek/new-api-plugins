...
// resolution, quality, and image_count as task billing facts.
...
export const meta = {
  apiVersion: 1,
  key: "gpt-image-pricing",
  name: "GPT Image Pricing",
  version: "1.0.0",
  icon: "OpenAI.Color",
  author: { name: "Airovo", url: "https://www.airovo.cn" },
  website: "https://platform.openai.com/docs/guides/images",
  description: {
    en: "OpenAI GPT Image generation and editing with resolution-aware billing",
    zh: "支持按分辨率计费的 OpenAI GPT Image 生图与编辑插件",
  },
  channelTypes: [1],
  ...
  usageSchema: {
    ...
    quality: {
      ...
      description: { en: "Image quality", zh: "图片质量" },
    },
  },
  usageExamples: [
    { label: "1K · standard · 1 image", facts: { image_count: 1, resolution: "1024x1024", quality: "medium" } },
    { label: "2K · high · 1 image", facts: { image_count: 1, resolution: "1536x1024", quality: "high" } },
  ],
  protocols: [
    {
      name: "openai_image",
...
function normalizeQuality(value) {
  ...
  if (!QUALITIES.includes(valueText)) throw new Error("quality must be low, medium, high, or auto");
  return valueText;
}
...
function factsFromRequest(ctx) {
  ...
  return { image_count: req.n, resolution: req.size, quality: req.quality };
}
...
export const protocols = {
  openai_image: {
    decodeRequest(ctx) {
      ...
      const normalized = { model, prompt, image: images[0], images, size: req.size || req.resolution, quality: req.quality, n: req.n, response_format: req.response_format };
