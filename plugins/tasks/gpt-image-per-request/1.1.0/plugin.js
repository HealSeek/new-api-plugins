...
export const meta = {
  ...
  usageExamples: [
    ...
    { label: "2K request", facts: { request_count: 1, resolution: "1536x1024" } },
  ],
  protocols: [
    {
  ...
  for (const item of value) {
    if (typeof item === "string") parts.push(item);
      ...
      for (const part of content) {
        ...
        else if (isObject(part) && (part.type === "input_text" || part.type === "text")) parts.push(text(part.text));
      }
    }
  }
...
function responseImageTool(req) {
  for (const tool of req.tools || []) {
    if (isObject(tool) && tool.type === "image_generation") return tool;
  }
  return {};
}
...
export const protocols = {
  openai_responses: {
    decodeRequest(ctx) {
      ...
      const tool = responseImageTool(req);
      const prompt = responseText(req.input) || text(req.prompt);
      ...
      return {
        ...
        requestBody: {
          model: ctx.model,
          prompt,
          size: tool.size || req.size,
          quality: tool.quality || req.quality,
          n: tool.n || req.n,
        },
      };
    },
    renderFinal(_ctx, task) {
      const data = task && task.data && Array.isArray(task.data.data) ? task.data.data : [];
      const content = data.map((item) => ({
        type: "output_text",
        text: item && item.url ? "![Generated image](<" + item.url + ">)" : String(item && item.b64_json ? item.b64_json : ""),
        annotations: [],
        logprobs: [],
      }));
      return { output: [{ type: "message", status: "completed", role: "assistant", content }], metadata: { vendor: "openai-image" } };
    },
  },
  openai_image: {
    decodeRequest(ctx) {
